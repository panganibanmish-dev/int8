...
<script type="text/javascript">
    jQuery(document).ready(function($){
        ...
        $(".row").on("click",".filter-btn",function(){
          ...
          $.ajax({
            type: "POST",
            url: "<?php echo base_url() ?>report/display_reports",
            data: {stageValues:stageValues,statusValues:statusValues,precinctValues:precinctValues,projectValues:projectValues,colorValues:colorValues,borderValues:borderValues},
            success : function(response){
              $("#bar-chart").remove();
              $(".bar-chart-container").html('<canvas id="bar-chart"></canvas>');
              $("#pie-chart").remove();
              $(".pie-chart-container").html('<canvas id="pie-chart"></canvas>');
             // alert(response);
              $(".loading").hide();
              var responseA = JSON.parse(response);
              if(responseA.status=="success"){
                  ...
                  $(".box-tools").show();
                  var cData = responseA.chart_data;
                  var chartType = responseA.chart_type;
                  var ctx = $("#bar-chart");
                  var ctx2 = $("#pie-chart");
                  if(chartType=='mixed'){
                    //bar chart data
